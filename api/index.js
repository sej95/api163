...
export const getSearchHot = async () => {
  return await api.get('/search/hot', {})
}
...
export const getPlayListDetail = async (params = {}) => {
  const { id, s = 8, timestamp = Date.now() } = params
  return await api.get('/playlist/detail', { 
    params: { id, s, timestamp } 
  })
}
 ...
 * @method 获取歌曲详情
 */
export const getSongDetail = async (ids, timestamp = Date.now()) => {
  return await api.post('/song/detail', { ids }, { 
    params: { timestamp } 
  })
}
 ...
 */
export const getVideoDetailInfo = async (vid, timestamp = Date.now()) => {
  return await api.get('/video/detail/info', { params: { vid, timestamp } })
}
 ...
 */
export const getMvDetailInfo = async (mvid, timestamp = Date.now()) => {
  return await api.get('/mv/detail/info', { params: { mvid, timestamp } })
}
